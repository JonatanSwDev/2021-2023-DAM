...
public class Labyrinth {
		...
		if (option > 0 && option <= maze.length) {
			...
			for (int i = 0; i < lines.size(); i++) {
				if (lines.get(i).length() != aux) {
					loaded = false;
					System.err
							.println("Se ha producido un error al cargar el laberinto\n Error 101: Laberinto corrupto");
					return;
				}
			...
			filename = maze[option - 1].getName().toString();
			SysLog.loadLaby(filename);
		} else {
	...
	public void setEntranceExit() throws InterruptedException {
		...
		if (startI >= map.length || startJ >= map.length || startI <= 0 || startJ <= 0) {
			System.err.println("¡Error! Casilla fuera de alcance");
			SysLog.errorENEX(startI, startJ, endI, endJ);
			return;
		}
		...
		if (endI >= map.length || endJ >= map.length || endI <= 0 || endJ <= 0) {
			System.err.println("¡Error! Casilla fuera de alcance");
			SysLog.errorENEX(startI, startJ, endI, endJ);
			return;
		}
		...
		try {
			...
			if (map[startI][startJ] != '#') {
				...
				SysLog.succesENEX(startI, startJ, endI, endJ);
			}
	...
	public void findFirst() throws InterruptedException {
		...
		if (!loaded) {
			...
			SysLog.errorSearching(loaded, establishedStEn);
		} else if (loaded && establishedStEn) {
			solved = findFirstPath();
			SysLog.solveShort(solved, path.size());
			if (solved) {
				showLabyrinth();
			}
	...
	public void findShort() throws InterruptedException {
		solved = false;
		if (!loaded) {
			System.err.println("\nAntes debe cargar un laberinto");
			SysLog.errorSearching(loaded, establishedStEn);
			...
			SysLog.solveFirst(solved, path.size());
			if (solved) {
				showLabyrinth();
			}
		} else if (!establishedStEn) {
	...
	private boolean findFirstPath() {
		if (!loaded) {
			System.err.println("No se ha cargado ningun laberinto");
		}
		boolean found = false;
		...
		while (!found && path.size() > 0) {
			...
			if (path.get(path.size() - 1).direction < 5) {
				...
				if (checkCell(nextCell)) { // Comprobar casilla válida
					path.add(nextCell);
					if (nextCell.i == endI && nextCell.j == endJ) { // Comprobamos si la siguiente celda councide con la
																	// salida!!!!
						found = true;
					}
				}
